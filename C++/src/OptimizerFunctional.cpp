...
#include "OptimizerFunctional.h"
#include "Simulator.h"

OptimizerFunctional::OptimizerFunctional(
    const ObservedPopulation& pop_observed_, const Population& pop_init_,
    const Vector& quarantine_input, int T_avg_, int T_unopt_,
    int max_iter_per_pass_, int max_passes_, int seed) :
    pop_observed(pop_observed_), pop_init(pop_init_),
    T_avg(T_avg_), T_unopt(T_unopt_),
    max_iter_per_pass(max_iter_per_pass_), max_passes(max_passes_),
//    nt_opt(pop_observed.getNumDays() + t_buffer),
    params(pop_observed.getNumDays(), T_unopt, quarantine_input),
    ...
    rand_engine(seed), uniform_rand(0,1)
{
  ...
  std::cout << "No. of days predicted: " << T_unopt_ << std::endl;
...
void
OptimizerFunctional::optimizeParametersNLOPT()
{
