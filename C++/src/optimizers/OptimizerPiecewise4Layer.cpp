...
#include "OptimizerPiecewise4Layer.h"
#include "models/Simulator.h"
#include "linearalgebra/SurrealS.h"

constexpr std::array<int,3> OptimizerPiecewise4Layer::IFR_SEG_STARTS;

OptimizerPiecewise4Layer::OptimizerPiecewise4Layer(
    const ObservedPopulation& pop_observed_, const Population4Layer<double>& pop_init_,
    ...
    params(pop_observed.getNumDays(), 0, vaccination_data),
...
void
OptimizerPiecewise4Layer::optimizeParametersNLOPT()
{
...
double
OptimizerPiecewise4Layer::getCostNLOPT(const std::vector<double>& x, std::vector<double>& grad, void* data)
{
  OptimizerPiecewise4Layer* opt = reinterpret_cast<OptimizerPiecewise4Layer*>(data);
...
OptimizerPiecewise4Layer::getCost(const ModelParams4Layer<T>& params_tmp)
{
  using namespace std;
...
double
OptimizerPiecewise4Layer::getCostGradientFD(
  const ModelParams4Layer<double>& params_orig, std::vector<double>& grad)
{
  ModelParams4Layer<double> params_tmp = params_orig; //create a copy of the current parameters
...
double
OptimizerPiecewise4Layer::getCostGradientAD(
  const ModelParams4Layer<double>& params_orig, std::vector<double>& grad)
{
  ModelParams4Layer<SurrealS<1,double>> params_tmp(params_orig); //create a copy of the current parameters
...
void
OptimizerPiecewise4Layer::updateOptimalSolution(
...
OptimizerPiecewise4Layer::copyVector2Param(const std::vector<T>& v, ModelParams4Layer<T>& params)
{
  ...
  for (int i = params.nt_hist; i < N; ++i) //Copy last "history" value to prediction section
  {
    ...
    params.c[i]    = params.c[params.nt_hist-1];
